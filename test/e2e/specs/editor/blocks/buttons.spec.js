...
test.describe( 'Buttons', () => {
	...
	test( 'has focus on button content', async ( { page, pageUtils } ) => {
		await pageUtils.insertBlock( { name: 'core/buttons' } );
		await page.keyboard.type( 'Content' );

		expect( await pageUtils.getEditedPostContent() ).toMatchSnapshot();
	} );

	test( 'has focus on button content (slash inserter)', async ( { page, pageUtils } ) => {
	...
	test( 'dismisses link editor when escape is pressed', async ( { page, pageUtils } ) => {
	...
	test( 'moves focus from the link editor back to the button when escape is pressed after the URL has been submitted', async ( { page, pageUtils } ) => {
	...
	test( 'can jump to the link editor using the keyboard shortcut', async ( { page, pageUtils } ) => {
		await pageUtils.insertBlock( { name: 'core/buttons' } );
		await page.keyboard.type( 'WordPress' );
		await pageUtils.pressKeyWithModifier( 'primary', 'k' );
